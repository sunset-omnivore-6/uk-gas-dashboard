...
def hex_to_rgba(hex_color, alpha=0.87):
...
def create_stacked_area_chart(dates, series, bounds, chart_type="stock"):
    ...
    for fac in ordered:
        ...
        fig.add_trace(go.Scatter(
            ...
            fillcolor=hex_to_rgba(fac["col"], 0.87),
            stackgroup='one',
...
def render_gas_storage_tab():
        ...
        with c1:
            chart_from = st.date_input(
                ...
                value=st.session_state.get(
                    "storage_from",
                    date.today() - timedelta(days=preset_options.get(
                        st.session_state.storage_preset, 365
                    ))
                ),
    ...
    st.markdown(
        ...
        f'Working Volumes — bounds: <strong style="color:#60A5FA">{bounds_from}</strong>'
        f' → <strong style="color:#60A5FA">{bounds_to}</strong></div>',
        unsafe_allow_html=True
    )
